...
输出（每行一条JSON，没有则输出空）：''';
  ...
  /// 获取群聊消息总数
  Future<int> getChatMessageCount(String groupChatId) async {
    try {
  ...
  /// 保存故事书对话摘要（使用最后 N 条消息）
  Future<String?> saveStoryConversationSummary({
    ...
    try {
      ...
      final rows = await db.query(
        'chat_messages',
        columns: ['content', 'senderName', 'isUser', 'createdAt'],
        where: 'chatId = ?',
      ...
      rows.sort((a, b) => ((a['createdAt'] as String?) ?? '').compareTo((b['createdAt'] as String?) ?? ''));
      final summary = rows.map((r) {
        final sender = r['senderName'] as String? ?? (r['isUser'] == 1 ? '用户' : '角色');
  ...
  /// 保存故事书章节（使用最后 N 条消息）
  Future<String?> saveStoryConversationChapter({
    required String groupChatId,
    int messageCount = 20,
  }) async {
    return saveStoryConversationSummary(
  ...
  /// 提取故事书记忆（返回是否成功）
  Future<bool> extractStoryMemory({
    required String groupChatId,
    required String userId,
    ...
    try {
      final summary = await saveStoryConversationSummary(groupChatId: groupChatId);

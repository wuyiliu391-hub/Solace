...
enum BtActionType {
  ...
  // ─── 设备操控 — UI 操作（走 AccessibilityService） ───
  phoneTap,
  phoneSwipe,
  phoneLongPress,
  phoneBack,
  ...
  phoneOpenApp,
  phoneScreenRead,
  phoneGetNotifications,
  phoneTakeScreenshot,
  phoneOpenNotificationsPanel,
  phoneQuickSettings,
...
const Map<BtActionType, BtPermissionCategory> btActionCategoryMap = {
  ...
  // ─── 设备操控 ───
  BtActionType.phoneTap: BtPermissionCategory.device,
  BtActionType.phoneSwipe: BtPermissionCategory.device,
  BtActionType.phoneLongPress: BtPermissionCategory.device,
  BtActionType.phoneBack: BtPermissionCategory.device,
  BtActionType.phoneHome: BtPermissionCategory.device,
  BtActionType.phoneRecentApps: BtPermissionCategory.device,
  BtActionType.phoneScroll: BtPermissionCategory.device,
  BtActionType.phoneTypeText: BtPermissionCategory.device,
  BtActionType.phoneClickText: BtPermissionCategory.device,
  BtActionType.phoneOpenApp: BtPermissionCategory.device,
  BtActionType.phoneScreenRead: BtPermissionCategory.device,
  BtActionType.phoneGetNotifications: BtPermissionCategory.device,
  BtActionType.phoneTakeScreenshot: BtPermissionCategory.device,
  BtActionType.phoneOpenNotificationsPanel: BtPermissionCategory.device,
  BtActionType.phoneQuickSettings: BtPermissionCategory.device,
  BtActionType.phoneSetWifi: BtPermissionCategory.device,
  BtActionType.phoneSetBluetooth: BtPermissionCategory.device,
  BtActionType.phoneSetVolume: BtPermissionCategory.device,
  BtActionType.phoneSetBrightness: BtPermissionCategory.device,
  BtActionType.phoneExecShell: BtPermissionCategory.device,
  BtActionType.phoneInstallApp: BtPermissionCategory.device,
  BtActionType.phoneUninstallApp: BtPermissionCategory.device,
  BtActionType.phoneGrantPermission: BtPermissionCategory.device,
};
...
- 普通聊天也必须包在 type=chat 的 JSON 中，不允许直接返回纯文本。
	...
	【设备操控动作说明】
	当设备操控总开关开启时，你可以使用以下 phone* 动作来操控用户手机：
	- phoneTap: 点击屏幕指定位置，value="x,y"（例如 "500,300"）
	- phoneSwipe: 滑动屏幕，value="x1,y1,x2,y2,durationMs"
	- phoneLongPress: 长按，value="x,y,durationMs"
	- phoneBack: 返回上一页
	- phoneHome: 回到桌面
	- phoneRecentApps: 打开最近任务
	- phoneTypeText: 在输入框打字，value="要输入的文字"
	- phoneClickText: 点击包含指定文字的按钮，value="按钮文字"
	- phoneOpenApp: 打开应用，value="包名（如 com.tencent.mm）"
	- phoneScreenRead: 读取当前屏幕内容
	- phoneGetNotifications: 读取通知栏
	- phoneScroll: 滚动屏幕，value="方向,x,y"（方向：UP/DOWN）
	
	如果 Shizuku 可用（需额外激活），还可以使用：
	- phoneSetWifi: 开关 WiFi，value="true/false"
	- phoneSetVolume: 设置音量，value="0-100"
	- phoneSetBrightness: 设置亮度，value="0-255"
	
	使用设备操控动作时，params.value 包含操控参数，params.reason 说明为什么执行此操作。

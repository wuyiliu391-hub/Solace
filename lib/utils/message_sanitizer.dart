...
class MessageSanitizer {
  ...
  /// AI 拒绝/安全拦截模板的宽泛识别。
  ///
  /// 与 chat_bloc_utils 里保守的 aiRefusalPattern（用于「是否重试换措辞」）不同，
  /// 本方法用于「上下文/记忆卫生」：只要疑似拒绝/脱角色模板，就不让它进入
  /// 历史上下文、会话状态锚点或记忆注入，避免某个模型拒绝一次后，后续（含换模型后）
  /// 的对话持续被这段拒绝文本限制。
  static final _aiRefusalPattern = RegExp(
    ...
    r'作为.*(?:AI|语言模型|人工智能)|很抱歉.*无法|抱歉.*暂时无法|抱歉.*现在无法|对不起.*(?:无法|不适合|不能)|'
    r'未成年|涉及.*(?:未成年|儿童|幼)|安全底线|必须遵照.*(?:底线|安全)|'
    r'我必须.*(?:拒绝|遵照)|无法生成.*(?:角色设定|内容)|'
    r'无法扮演此角色|不能继续扮演|不能.*角色扮演|拒绝.*扮演|'
    r'(?:AI|语言模型|人工智能|助手).{0,12}(?:无法|不能|拒绝).{0,20}(?:角色扮演|扮演|生成|继续)|'
    r'(?:内容政策|安全限制|使用条款).{0,10}(?:不允许|禁止|限制)|'
    r'超出.{0,6}(?:训练|设计|能力).{0,6}范围|'
    // 助手身份自述 / 客服用语（无拒绝动词也判脱角色，IDE 反代模型常见）
    r'我是(?:AI|人工智能|语言模型|聊天机器人)(?:助手)?|'
    r'我是助手|我是你的(?:AI)?助手|我只是(?:一个)?(?:AI|程序|模型|聊天机器人|助手)|'
    r'有什么(?:可以帮|能帮)你的吗|很高兴为你服务|随时(?:准备)?(?:为你|帮您)服务',
    caseSensitive: false,
  );

  static bool isAIRefusal(String content) {
    final text = sanitizeFinal(content).trim();
    if (text.isEmpty) return false;
    return _aiRefusalPattern.hasMatch(text);
  }
